...
{
    char* p; // Used by strtol
    int fileAt; // Marks the input file location in argv
    FILE *fin = NULL;
    char buff[BUFFER_SIZE], event='\0';

    /* Check command buff arguments and get MG */
    switch (argc) {
        /* Command: run <MG> <file> */
        case 3:
            MG = strtol(argv[1], &p, 10);
            if (MG < 1) {
                fprintf(stderr, "Max Groups (MG) must be more than zero.\n");
                return EXIT_FAILURE;
            }
            fileAt = 2;
            break;
        ...
        case 2:
            ...
            break;
        default:
            fprintf(stderr, "Usage: %s <max groups> <input_file> OR %s <input_file>\n", argv[0], argv[0]);
            return EXIT_FAILURE;
    }

    /* Open input file */
    if ((fin = fopen(argv[fileAt], "r")) == NULL)
    {
